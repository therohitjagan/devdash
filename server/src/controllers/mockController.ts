...
export async function listMocks(req: Request, res: Response) {
  if (!req.user) {
...
export async function createMock(req: Request, res: Response) {
  if (!req.user) {
    res.status(401).json({ message: 'Authentication required' })
    return
  }
  ...
  const mock = await prisma.mockApi.create({
    data: {
      ...
      response: incoming.response as object,
      userId: req.user.sub,
    },
  })
...
export async function deleteMock(req: Request, res: Response) {
  if (!req.user) {
    res.status(401).json({ message: 'Authentication required' })
    return
  }
...
export async function exportMocksOpenApi(req: Request, res: Response) {
  if (!req.user) {
    res.status(401).json({ message: 'Authentication required' })
    return
  }
  ...
  const paths = mocks.reduce<Record<string, Record<string, unknown>>>((acc, mock) => {
...
export async function exportMocksPostman(req: Request, res: Response) {
  if (!req.user) {
    res.status(401).json({ message: 'Authentication required' })
    return
  }
