...
export type CommitAnalysisJob = {
  repoUrl: string
}

export const commitAnalysisQueue = new Queue<CommitAnalysisJob, unknown, 'analyze'>('commit-analysis', {
  connection: {
    url: env.REDIS_URL,
  },
})
...
export function startCommitAnalysisWorker() {
  return new Worker<CommitAnalysisJob>(
    'commit-analysis',
    async (job) => {
